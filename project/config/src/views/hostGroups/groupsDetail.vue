    ...
    .sf-panel
      .panel-body
        ...
        sfTable(:tColumns='tColumns',:tData="tData",:tableSearch.sync="tableSearch",type="selection",@search="search")
          el-table-column(slot="a",label='主机名',width="200px")
            template(slot-scope="scope")
              router-link(:to="'/host/hosts/hostDetail/'+scope.row.id") {{scope.row.name}}
...
export default {
  ...
  data() {
    return {
      ...
      tColumns: [
          // { prop: 'hostname', label: '主机名', sortable: 'custom' },
          { prop: 'ip', label: 'IP地址' },
        //   { prop: 'bind', label: 'Agent心跳' },
          { prop: 'cpu', label: 'CPU' },
          { prop: 'memory', label: '内存' },
          { prop: 'hd', label: '磁盘' },
          { prop: 'stat', label: '状态' },
          { prop: 'hostgroup', label: '所属分组' },
          { prop: 'cluster', label: '所属集群' },
      ],
      tData: [
